...
@NamedQueries({
	@NamedQuery(name = "OrderDetail.countBook",
			query = "SELECT COUNT(od) FROM OrderDetail od WHERE od.book.bookId =:bookId"),
	@NamedQuery(name = "OrderDetail.bestSellerBook", query = "SELECT od.book, SUM(od.quantity) AS totalQuantity FROM OrderDetail od GROUP BY od.book.bookId ORDER BY totalQuantity DESC"),
})
